...
export function ScorePreview({ answered, total, onReview, onOverview }: Props) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
      <section role="status" aria-label="สถานะการส่งการประเมิน" data-testid="success-container" className="overflow-hidden rounded-[2rem] bg-slate-950 p-7 text-white shadow-xl sm:p-10">
        <div className="flex flex-wrap items-start justify-between gap-6">
          ...
          <div className="min-w-48 rounded-2xl border border-white/10 bg-white/[0.07] p-5 text-right"><p className="text-xs uppercase tracking-[0.16em] text-slate-400">Interim score</p><p data-testid="interim-score" className="font-display mt-2 text-4xl font-semibold text-indigo-300">{score.toFixed(2)}</p><p className="mt-1 text-sm text-slate-400">จาก 20 คะแนน</p></div>
        </div>
      </section>

      <section className="grid gap-4 sm:grid-cols-3">
        <article data-testid="group-quality-card" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.15em] text-slate-400">คุณภาพกลุ่ม</p><p data-testid="group-quality-score" className="font-display mt-3 text-3xl font-semibold text-slate-950">12.80</p><p className="mt-1 text-sm text-slate-500">จาก 15 · ไม่ถูกลดเพราะสมาชิกคนอื่น</p></article>
        <article data-testid="individual-card" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.15em] text-slate-400">รายบุคคล</p><p data-testid="individual-score" className="font-display mt-3 text-3xl font-semibold text-slate-950">4.13</p><p className="mt-1 text-sm text-slate-500">จาก 5 · band mapping 60–100%</p></article>
        <article data-testid="participation-card" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.15em] text-slate-400">Participation</p><p data-testid="participation-score" className="font-display mt-3 text-3xl font-semibold text-slate-950">{Math.round(participation * 100)}%</p><p className="mt-1 text-sm text-slate-500">Multiplier {multiplier.toFixed(2)} · threshold 90%</p></article>
      </section>
      ...
      <div className="flex flex-col gap-3 sm:flex-row"><button type="button" onClick={onReview} data-testid="review-button" className="min-h-12 flex-1 rounded-xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-800 hover:bg-slate-50">ทบทวนคำตอบ</button><button type="button" onClick={onOverview} data-testid="overview-button" className="min-h-12 flex-1 rounded-xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white hover:bg-slate-800">กลับหน้าภาพรวม</button></div>
    </div>
  )
}
